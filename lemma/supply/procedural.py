...
def _generate_depth2_candidates_sequential(
    ...
    *,
    ...
    source_family_limit = _accepted_source_family_limit(ctx, count=count, required_count=required_count)
    cursor = 0
...
def _accepted_source_family_limit(ctx: _Depth2GenerationContext, *, count: int, required_count: int) -> int:
    family_counts = Counter(_source_family(source) for source in ctx.ordered)
    if len(family_counts) <= 1:
        return 0
    limit = max(2, math.ceil(count / 3))
    return limit if sum(min(size, limit) for size in family_counts.values()) >= required_count else 0
