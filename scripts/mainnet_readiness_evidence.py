...
def run_audit(
    *,
    ...
    checklist_plan = {
        ...
        "2": [
            """uv run python - <<'PY'\nfrom pathlib import Path\nimport os\n\n\ndef env_value(line: str, key: str) -> str | None:\n    marker = f'{key}='\n    if marker not in line:\n        return None\n    value = line.split(marker, 1)[1].strip()\n    if not value:\n        return None\n    value = value.split()[0]\n    return value.strip('\"').strip(\"'\")\n\nsystemd_root = Path('/etc/systemd/system')\npatterns = (\n    'lemma-validator*.service',\n    'lemma-active-registry-prebuild.service',\n    'lemma-validator-bucket.service',\n    'lemma-publisher*.service',\n)\nservice_paths = sorted({path for pattern in patterns for path in systemd_root.glob(pattern)})\nif not service_paths:\n    raise SystemExit('no runtime service units found for single-validator-path validation')\n\nstale: list[str] = []\nexplicit_role = os.environ.get('LEMMA_ACTIVE_REGISTRY_ROLE')\nif explicit_role:\n    print(f'process:LEMMA_ACTIVE_REGISTRY_ROLE={explicit_role}')\n    stale.append('process')\n\nfor unit_path in service_paths:\n    text = unit_path.read_text(encoding='utf-8', errors='ignore')\n    values = [\n        value for line in text.splitlines() for value in [env_value(line, 'LEMMA_ACTIVE_REGISTRY_ROLE')] if value\n    ]\n    if not values:\n        print(f'{unit_path.name}:single_validator_path=ok')\n        continue\n    for value in values:\n        print(f'{unit_path.name}:LEMMA_ACTIVE_REGISTRY_ROLE={value}')\n        stale.append(unit_path.name)\n\nif stale:\n    raise SystemExit('stale validator role settings: ' + ', '.join(sorted(set(stale))))\n\nprint('single_validator_path=pass')\nprint('result=pass')\nPY""",
        ],
        "3": [
            ...
            "grep -n \"LEMMA_ENABLE_SET_WEIGHTS\\|LEMMA_ENABLE_SET_COMMITMENT\\|LEMMA_HOTKEY\\|LEMMA_ACTIVE_REGISTRY_CACHE_DIR\\|LEMMA_GIT_SHA\" /etc/systemd/system/lemma-validator*.service 2>/dev/null || true",
        ...
        "5": [
            ...
            "ls -l scripts/lemma-sync-active-registry-cache lemma/cli/main.py scripts/publish_proof_atlas_snapshot.py",
            "test -x scripts/lemma-sync-active-registry-cache || true",
            "test -f scripts/lemma-sync-active-registry-cache",
            "test -f scripts/publish_proof_atlas_snapshot.py && test -f lemma/cli/main.py",
        ],
        "7": [
            "uv run python scripts/pre_mainnet_checklist.py",
            "uv run python scripts/pre_mainnet_checklist.py --json",
            "uv run python scripts/publish_proof_atlas_snapshot.py --repo \"${LEMMA_PROOF_ATLAS_REPO}\" --netuid \"sn${BT_NETUID}\" --sync-proof-dir \"${LEMMA_CORPUS_OUTPUT_DIR}\" --sync-canonical-dir \"${LEMMA_CANONICAL_OUTPUT_DIR}/sn${BT_NETUID}\" --sync-registry-cache-dir \"${LEMMA_ACTIVE_REGISTRY_CACHE_DIR}\" --dry-run",
            "uv run python scripts/publish_chain_commitment.py --repo \"${LEMMA_PROOF_ATLAS_REPO}\" --netuid \"sn${BT_NETUID}\" --bt-netuid \"${BT_NETUID}\" --readback --hotkey \"${VALIDATOR_HOTKEY}\"",
