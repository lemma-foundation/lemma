...
Production validation fails unless `LEMMA_TASK_SUPPLY_MODE=registry`, the registry bytes match `LEMMA_TASK_REGISTRY_SHA256_EXPECTED`, paid tasks are real missing-proof rows with public source references, Lean verifier networking is disabled, live miner authentication is required, commit/reveal fields are required, and strong proof identity is required for paid rewards. Accepted proofs record their actual Lean kernel dependencies, and rewarded slot weights come from those recorded dependencies plus the deterministic active-slot prior.
...
For the production loop, pass `--bucket-reveals-dir` for the live reveal inbox, or `--bucket-reveals-jsonl` for a fixture file. One validator pass scores one completed reveal tempo: production mode idles on current or future reveal tempos until the chain advances. Directory intake selects the newest completed tempo, moves successfully processed files to `processed/`, and moves older top-level reveal files to `stale/`. The validator recomputes each miner's Merkle root from `(slot_index, ciphertext_sha256)` pairs before turning revealed proofs into submissions. Add `--verify-chain-commitments` to read the miner's on-chain bucket commitment, and add `--verify-drand-reveals` to decrypt each bucket ciphertext and require it to match the revealed proof; production mode enables both checks for bucket reveals. Binary ciphertexts should be JSON-encoded as `base64:<payload>` or `0x<hex>`. To poll live buckets directly, pass `--miner-buckets-json`, `--bucket-commit-blocks-json`, `--bucket-drand-round`, and `--bucket-drand-signature`; in production this polls the most recent completed tempo and reads each miner commitment at its declared commit block. Miners can build those public `tempo_<t>/slot_<i>.bin` objects with `lemma miner bucket publish`, and the validator fetches them before reusing the same reveal validation path.
...
Run `uv run python scripts/refresh_site_current_problems.py --site-repo /opt/lemmasub.net --current-cache-dir /var/lib/lemma-active-registries --keep-output-when-current-cache-missing --skip-randomness-hashes --commit --push` from the validator-side publish timer to refresh the public website's active-problem dashboard.
