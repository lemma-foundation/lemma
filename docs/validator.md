...
When `--set-weights` is enabled, each chain-write attempt also appends a public-safe `weight-submissions.jsonl` receipt under `LEMMA_OPERATOR_DATA_DIR` with the resolved UID vector, weights, network, netuid, success flag, sanitized client message, and extrinsic hash when available. When `--set-commitment` is enabled with `LEMMA_ENABLE_SET_COMMITMENT=1`, the validator submits the compact tempo commitment binding active-pool digest, accepted-entry digest, and accepted Merkle root.
...
Production validation fails unless `LEMMA_TASK_SUPPLY_MODE=procedural`, the public source pool is SHA-pinned, `LEMMA_PROCEDURAL_PRIOR_CORPUS_DIR` points at the prior accepted-entry substrate mirror, paid tasks are procedural depth-2 and generated from chain/drand epoch randomness with the chain-pinned operator bundle, drand-keyed mutation params, public source-pool receipts, public novelty-cache receipts, Lean-printed `kernel_canonical_hash`, and public import-graph slot-weight receipts, Lean verifier networking is disabled, live miner authentication is required, commit/reveal fields are required, and strong proof identity is required for paid rewards. Registry files are caches for distribution and replay, not production authority.
...
For the production loop, pass `--bucket-reveals-jsonl` with post-reveal miner bucket artifacts. The validator recomputes each miner's Merkle root from `(slot_index, ciphertext_sha256)` pairs before turning revealed proofs into submissions. Add `--verify-chain-commitments` to read the miner's on-chain bucket commitment, and add `--verify-drand-reveals` to decrypt each bucket ciphertext and require it to match the revealed proof; production mode enables both checks for bucket reveals. Binary ciphertexts should be JSON-encoded as `base64:<payload>` or `0x<hex>`. To poll live buckets directly, pass `--miner-buckets-json`, `--bucket-drand-round`, and `--bucket-drand-signature`; the validator fetches canonical slot objects and then reuses the same reveal validation path.
