...
Production validation fails unless `LEMMA_TASK_SUPPLY_MODE=procedural`, the public source pool is SHA-pinned, paid tasks are procedural depth-2 and generated from chain/drand epoch randomness with the chain-pinned operator bundle and drand-keyed mutation params, Lean verifier networking is disabled, live miner authentication is required, commit/reveal fields are required, and strong proof identity is required for paid rewards. Registry files are caches for distribution and replay, not production authority.
