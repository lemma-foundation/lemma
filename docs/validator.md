...
Production validation fails unless `LEMMA_TASK_SUPPLY_MODE=procedural`, the public source pool is SHA-pinned, paid tasks are procedural depth-2 and generated from chain/drand epoch randomness with the chain-pinned operator bundle, drand-keyed mutation params, and public novelty-cache receipts, Lean verifier networking is disabled, live miner authentication is required, commit/reveal fields are required, and strong proof identity is required for paid rewards. Registry files are caches for distribution and replay, not production authority.
