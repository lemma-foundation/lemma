...
Lean theorem proving is the only production domain for now. Math is the wedge; verified data is the product.
...
Lemma uses normal Bittensor miner and validator emissions. It does not use smart contracts, escrow custody, owner-cut routing, or contract-routed payouts.

Google DeepMind Formal Conjectures is a frontier benchmark and downstream demonstration layer. It is not the base reward stream, and Lemma is not endorsed by Google DeepMind.
