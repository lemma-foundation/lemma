...
```bash
uv run python scripts/refresh_site_current_problems.py --site-repo /opt/lemmasub.net --current-cache-dir /var/lib/lemma-active-registries --keep-output-when-current-cache-missing --skip-randomness-hashes --commit --push
```
