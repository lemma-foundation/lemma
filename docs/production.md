...
LEMMA_CURRICULUM_STATE_JSONL=canonical/sn467/curriculum/curriculum.jsonl
...
`LEMMA_CURRICULUM_RETARGET=1` makes `K` and frontier depth dynamic. Each validator pass writes the next retarget row into the canonical public curriculum artifacts, and later tempos use the latest prior row. In production, `LEMMA_CURRICULUM_STATE_PUBLIC=1` is required and `LEMMA_CURRICULUM_STATE_JSONL` should point at a state file synced from the public corpus artifacts; a private local curriculum row can make miners and validators drift.
