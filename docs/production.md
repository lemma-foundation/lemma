...
`LEMMA_CURRICULUM_RETARGET=1` is for development and operator simulation until the retarget state is published and replayable from public artifacts. Production burn-in should use fixed public `LEMMA_ACTIVE_K` and `LEMMA_FRONTIER_DEPTH`; otherwise miners and validators can drift when only one operator has the latest local curriculum row.
