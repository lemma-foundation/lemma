...
`LEMMA_CURRICULUM_RETARGET=1` makes `K` and frontier depth dynamic. Each validator pass writes the next retarget row into the canonical public curriculum artifacts. Later active windows use the latest eligible public row where `record.tempo < active_tempo - 1`, giving the row one full tempo of public replay lag before it can affect the active set. In production, `LEMMA_CURRICULUM_STATE_PUBLIC=1` is required and `LEMMA_CURRICULUM_STATE_JSONL` must point at a replay cache synced from the public corpus artifacts, outside `LEMMA_CANONICAL_OUTPUT_DIR`/`LEMMA_OPERATOR_DATA_DIR/canonical`; a private local row or a replay path inside canonical publish output can make miners and validators drift.
