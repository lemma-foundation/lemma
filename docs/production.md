...
If `LEMMA_ACTIVE_REGISTRY_CACHE_INDEX_URL` points at a public corpus `registries/<netuid>/index.json`, miners, validators, and cache warmers hydrate the tempo cache from that public mirror before falling back to local generation. The downloaded registry is still treated only as a cache: it must match the published SHA256 and the effective public curriculum state before it is written locally. Set `LEMMA_ACTIVE_REGISTRY_ROLE=auditor` on validators that should audit a public cache instead of generating locally; auditor mode uses a current hydrated cache when available and otherwise idles/fails closed rather than spending Lean compute.

`LEMMA_CURRICULUM_RETARGET=1` makes `K`, frontier depth, and the target subnet tempo dynamic. Solve-rate history moves frontier depth. Validator capacity and the optional curriculum cost budget cap `K`, so harder frontiers can automatically run fewer tasks. The tempo policy then gives harder or larger sets more blocks instead of forcing every set into the same fixed cadence. Each validator pass writes the next retarget row into the canonical public curriculum artifacts, including `active_window_blocks`; in chain mode this is the desired future `tempo` in blocks. Later active sets use the latest eligible public row where `record.tempo < active_tempo - 1`, giving the row one full tempo of public replay lag before it can affect active selection. In production, `LEMMA_CURRICULUM_STATE_PUBLIC=1` is required and `LEMMA_CURRICULUM_STATE_JSONL` must point at a replay cache synced from the public corpus artifacts, outside `LEMMA_CANONICAL_OUTPUT_DIR`/`LEMMA_OPERATOR_DATA_DIR/canonical`; a private local row or a replay path inside canonical publish output can make miners and validators drift.

To submit the retargeted chain tempo from a validator pass, enable it explicitly:

```bash
LEMMA_ENABLE_SET_TEMPO=1 uv run lemma validate --once --set-tempo --set-weights --set-commitment
```

The command uses `btcli sudo set --param tempo --value <blocks>` after scoring the completed tempo. It skips the write when the subnet already has the target tempo. The subnet owner/root permission model is enforced by Bittensor.
