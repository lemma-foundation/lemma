...
Formal Conjectures is not the payout path. It is useful as a downstream demonstration layer: train or tune provers on Lemma Proof Atlas exports, then measure whether they solve more frontier formal statements.
...
Accepted proof rows can be frozen into a compact JSONL export:

```bash
uv run lemma corpus benchmark-export --input proofs/sn467/accepted --output exports/lemma-proofs.jsonl --index exports/index.json
```
