...
7. No accepted corpus row without task id, task version, target hash, verifier version, task-source context, and validator attribution.
8. No verifier run with network access in production mode.
9. No validator scoring in production mode without a SHA-pinned public source pool.
10. No paid production problem set chosen by a registry publisher.
...
13. No paid production task unless supply is procedural depth-2 and generated from block-hash epoch randomness.
14. No paid production task without novelty, clean license state, and deterministic slot-weight metadata.
15. No production paid reward under weak proof identity.

`LEMMA_PROTOCOL_MODE=production` fails closed if the active configuration violates the launch boundary: enabled domains must be exactly `lean`, the source pool must be SHA-pinned, paid tasks must be procedural depth-2 and generated from the epoch's Bittensor block hash, live miner authentication and commit/reveal fields must be required, strong proof identity must be required for reward, and the Lean verifier network mode must be disabled. Direct file submissions satisfy this by hotkey signature; bucket-path submissions satisfy it by a matching miner chain commitment plus commit/reveal metadata.
