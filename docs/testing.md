...
It exercises the pinned-registry validator path with a fixed local task.
The production-like smoke in `tests/test_operator_registry_flow.py` additionally covers registry SHA pinning, signed revealed submissions, strong Lean proof-term identity, production preflight, diagnostics, scoring, and accepted-proof validation.
