...
When `LEMMA_ACTIVE_REGISTRY_CACHE_DIR` is set, miners and validators load `tempo-<tempo>.registry.json` for the active tempo if it exists. `LEMMA_ACTIVE_REGISTRY_JSON` pins one exact registry file and fails closed if that file is missing. Builder validators use the same deterministic generation path as the local warm command. Auditor validators set `LEMMA_ACTIVE_REGISTRY_ROLE=auditor`; they hydrate the public cache, verify it against the published hash and effective curriculum state, and refuse local generation if the current cache is not available.
