...
This is the end-to-end operator path for deriving active problems, validating submissions, and exporting accepted proof data.

## 1. Prepare Source Pool
...
Production validators pin the source-pool JSONL, not a paid-problem registry:

```bash
sha256sum snapshot.jsonl
```

Local smoke can still use a registry artifact:
...
The command writes deterministic `queue_position` values and prints `registry_sha256`. Keep registry artifacts for local replay and smoke tests only.

## 2. Configure Active Generation

Production active problems come from:

```text
pinned source pool + Bittensor epoch boundary block hash
```

Configure production validators with the same source pool and block-hash seed mode:
...
`LEMMA_ACTIVE_K` is validator throughput. `LEMMA_FRONTIER_DEPTH` controls difficulty. Payment uses deterministic active slot weights, not subjective validator scores.
...
The command fails if production source-pool pinning is missing, the active window cannot fill `K`, output directories cannot be prepared, or the Lean verifier backend is not configured.
...
The diagnostics file contains the preflight report, active task ids, artifact counts for validator runs, and no environment variables, credentials, wallet names, hostnames, IPs, or local filesystem paths.
...
Run `uv run python scripts/leak_check.py` before publishing. Do not publish wallets, local receipts, caches, logs, machine paths, private state, or unsanitized operator context.
