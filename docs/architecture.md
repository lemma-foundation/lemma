...
`LEMMA_PROTOCOL_MODE=production` fails closed unless `LEMMA_ENABLED_DOMAINS` is exactly `lean`, procedural supply uses a pinned public source pool, public novelty cache, and public import graph, paid tasks are depth-2 and generated from chain/drand epoch randomness with the chain-pinned operator bundle and drand-keyed params, live miner submissions are hotkey-authenticated, commit/reveal fields are required, strong proof identity is required for reward, and Lean verifier networking is disabled. File submissions authenticate by signature; bucket-path submissions authenticate by the miner's chain commitment.
