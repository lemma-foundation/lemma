...
For production-shaped supply, validators rebuild deterministic depth-2 procedural rows from public inputs before configuring the active pool. Each row carries the operator-bundle version, bundle hash, ordered operators, drand-keyed params, input/output statement hashes, and a public novelty-cache receipt. The detailed operator path lives in [Operator Registry Flow](docs/operator-registry-flow.md). Curated Mathlib and mixed supply remain useful for local smoke and curriculum development. SN467 burn-in uses the paid mainnet supply path on the test chain.
