...
Use [examples/operator-smoke](examples/operator-smoke/README.md) to build a local smoke task set, package one proof submission, run a validator pass, and export corpus artifacts.

For production, configure the same pinned source pool on every validator and set `LEMMA_ACTIVE_EPOCH_RANDOMNESS_SOURCE=chain_block_hash`. The detailed operator path lives in [Operator Flow](docs/operator-registry-flow.md). Curated Mathlib and mixed supply remain useful for local smoke and curriculum development.
...
- [Operator flow](docs/operator-registry-flow.md)
