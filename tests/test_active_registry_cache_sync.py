...
def test_sync_active_registry_cache_replaces_stale_existing_cache(
    ...
    stale_task = make_task(
        task_id="lemma.procedural.stale",
        title="Stale",
        theorem_name="stale",
        type_expr="True",
        source_stream="procedural",
        source_name="pytest",
        frontier_depth=0,
        metadata={**fresh_metadata, "source_sampling_version": "old"},
    )
    public_task = make_task(
        task_id="lemma.procedural.public",
        title="Public",
...
def test_sync_active_registry_cache_is_silent_without_public_index_or_local_cache(
    monkeypatch,
    tmp_path: Path,
    capsys,
) -> None:
...
def test_sync_active_registry_cache_accepts_current_local_cache_without_public_index(
    monkeypatch, tmp_path: Path, capsys
) -> None:
    module = _load_sync_module()
    cache = tmp_path / "cache"
